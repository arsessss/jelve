...
const Contact = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const validatedData = contactSchema.parse({ name, phone, message });
    ...
    <div className="min-h-screen bg-background">
            ...
            <Card className="max-w-2xl mx-auto p-8 animate-scale-in transition-all duration-500 hover:shadow-lg">
              <form onSubmit={handleSubmit} className="space-y-6" dir="rtl">
                <div>
                  ...
                  <Input value={name} onChange={(e) => setName(e.target.value)} required className="text-right" placeholder="نام خود را وارد کنید" />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-2">شماره تماس</label>
                  <Input value={phone} onChange={(e) => setPhone(e.target.value)} required type="tel" className="text-left" placeholder="09123456789" dir="ltr" />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-2">پیام</label>
                  <Textarea value={message} onChange={(e) => setMessage(e.target.value)} required rows={6} className="text-right resize-none" placeholder="پیام خود را بنویسید..." />
                </div>
                <Button type="submit" className="w-full bg-primary text-primary-foreground font-bold gap-2" disabled={loading}>
                  {loading ? (
      ...
      <footer className="bg-card border-t border-border py-8">
        <div className="container mx-auto px-4 text-center">
          <p className="text-muted-foreground animate-fade-in" dir="rtl">© ۱۴۰۴ مجتمع آموزشی جلوه. تمامی حقوق محفوظ است.</p>
        </div>
      </footer>
    </div>
  );
