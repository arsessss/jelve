...
import { 
  ...
  Edit2, Home, Newspaper, Image as ImageIcon, Shield, ClipboardList, Eye
} from "lucide-react";
...
const Admin = () => {
  ...
  const checkAuth = async () => {
    ...
    if (!valid || !session) {
      toast({ title: "نشست نامعتبر", description: "لطفا دوباره وارد شوید", variant: "destructive" });
      navigate("/login");
      return;
    }

    if (session.role !== "admin") {
      toast({ title: "دسترسی غیرمجاز", description: "شما دسترسی به این صفحه ندارید", variant: "destructive" });
      navigate("/");
      return;
    }
  ...
  const fetchStudentPeriodGrades = async (studentId: string, periodId: string) => {
    const { data, error } = await secureApi.select<StudentPeriodGrade>('student_period_grades', { student_id: studentId, period_id: periodId });
    if (!error && data) {
      const grades: Record<string, string> = {};
      data.forEach((g: StudentPeriodGrade) => { grades[g.subject] = g.grade || ""; });
  ...
  const deleteMessage = async (messageId: string) => {
    ...
    if (error) { toast({ title: "خطا", description: "حذف پیام با مشکل مواجه شد", variant: "destructive" }); }
    else { toast({ title: "حذف شد", description: "پیام با موفقیت حذف شد" }); fetchMessages(); }
  };
  ...
  const createUser = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const { userId, error: authError } = await customAuth.createUser(newStudent.username, newStudent.password, newStudent.name, newStudent.role);
      if (authError) { toast({ title: "خطا", description: authError, variant: "destructive" }); setLoading(false); return; }
      if (userId) {
        if (newStudent.role === "student") {
          const { error: studentError } = await secureApi.insert('students', { user_id: userId, full_name: newStudent.name, grade: newStudent.grade });
          if (studentError) { toast({ title: "خطا", description: studentError, variant: "destructive" }); setLoading(false); return; }
        }
        toast({ title: "موفقیت‌آمیز", description: newStudent.role === "admin" ? "ادمین با موفقیت ایجاد شد" : "دانش‌آموز با موفقیت ایجاد شد" });
  ...
  const deleteStudent = async (studentId: string, userId: string) => {
    ...
    try {
      ...
      fetchStudents();
    } catch { toast({ title: "خطا", description: "حذف دانش‌آموز با مشکل مواجه شد", variant: "destructive" }); }
  };
  ...
  const handleEditUser = async () => {
    ...
    try {
      ...
      if (editUserForm.username.trim()) {
        await secureApi.update('custom_users', editingStudent.user_id, { username: editUserForm.username });
      }
      ...
      if (editUserForm.password.trim()) {
        ...
        if (session) {
          const { data: result, error } = await supabase.functions.invoke('auth-change-password', {
            body: { token: session.token, target_user_id: editingStudent.user_id, new_password: editUserForm.password },
          });
          if (error || result?.error) {
            toast({ title: "خطا", description: result?.error || "تغییر رمز عبور ناموفق بود", variant: "destructive" });
            setEditUserLoading(false);
            return;
          }
        }
      }
      toast({ title: "موفق", description: "اطلاعات کاربر ویرایش شد" });
      setEditUserDialogOpen(false);
      fetchStudents();
    } catch {
      toast({ title: "خطا", description: "ویرایش کاربر ناموفق بود", variant: "destructive" });
  ...
  const createPeriodForStudent = async () => {
    ...
    if (error) { toast({ title: "خطا", description: error, variant: "destructive" }); }
    else { toast({ title: "موفق", description: "دوره نمره جدید ایجاد شد" }); setNewPeriodTitle(""); fetchGradePeriods(); }
    setLoading(false);
  };
  ...
  const deleteGradePeriod = async (periodId: string) => {
    ...
    if (error) { toast({ title: "خطا", description: error, variant: "destructive" }); }
    else { toast({ title: "حذف شد", description: "دوره نمره حذف شد" }); fetchGradePeriods(); }
  };
  ...
  const saveStudentPeriodGrades = async () => {
    ...
    try {
      for (const subject of SUBJECT_OPTIONS) {
        ...
        const { error } = await secureApi.upsert('student_period_grades', { student_id: selectedStudentForGrades.id, period_id: selectedPeriod.id, subject: subject.value, grade: gradeValue || null });
        if (error) throw new Error(error);
      }
      toast({ title: "موفق", description: "نمرات ذخیره شد" });
      setGradeDialogOpen(false);
    } catch { toast({ title: "خطا", description: "ذخیره نمرات با مشکل مواجه شد", variant: "destructive" }); }
    finally { setLoading(false); }
  };
  ...
  const deleteOnlineClass = async (classId: string) => {
    ...
    if (error) { toast({ title: "خطا", description: "حذف کلاس با مشکل مواجه شد", variant: "destructive" }); }
    else { toast({ title: "حذف شد", description: "کلاس با موفقیت حذف شد" }); fetchOnlineClasses(); }
  };

  const createJozveh = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!jozvehFile) { toast({ title: "خطا", description: "لطفاً یک فایل انتخاب کنید", variant: "destructive" }); return; }
    setLoading(true);
    try {
      ...
      const { error } = await secureApi.insert('jozveh', { grade: newJozveh.grade, subject: newJozveh.subject, title: newJozveh.title, link: urlData.publicUrl, file_url: urlData.publicUrl });
      if (error) throw new Error(error);
  ...
  const deleteJozveh = async (jozvehId: string) => {
    ...
    if (error) { toast({ title: "خطا", description: "حذف جزوه با مشکل مواجه شد", variant: "destructive" }); }
    else { toast({ title: "حذف شد", description: "جزوه با موفقیت حذف شد" }); fetchJozveh(); }
  };
  ...
  const handleEditPish = (pish: PishSabtenamData) => {
    setEditingPish(pish);
    setPishForm({ title: pish.title, content: pish.content });
    setPishImage(null);
  };
  ...
  const handleSavePish = async () => {
    ...
    try {
      ...
      if (pishImage) {
        ...
        const { error: uploadError } = await supabase.storage.from("profile-pictures").upload(fileName, pishImage);
        if (uploadError) throw uploadError;
      ...
      toast({ title: "موفق", description: "اطلاعات ذخیره شد" });
      setEditingPish(null);
      fetchPishSabtenam();
    } catch { toast({ title: "خطا", description: "ذخیره ناموفق بود", variant: "destructive" }); }
    finally { setPishLoading(false); }
  };

  const togglePishEnabled = async (pish: PishSabtenamData) => {
    await secureApi.update('pish_sabtenam', pish.id, { is_enabled: !pish.is_enabled });
    fetchPishSabtenam();
  };
  ...
  const sidebarItems = [
    ...
    { id: "roles" as ActiveSection, icon: Shield, label: "نقش‌ها" },
          ...
          <aside className="w-full lg:w-64 bg-card border-b lg:border-b-0 lg:border-l border-border p-3 lg:p-6 lg:sticky lg:top-24 lg:h-[calc(100vh-6rem)]" dir="rtl">
            <div className="flex lg:flex-col gap-2 overflow-x-auto lg:overflow-x-visible pb-2 lg:pb-0 scrollbar-hide">
              ...
              <button
                ...
                className="flex items-center gap-3 px-3 py-2.5 lg:px-4 lg:py-3 rounded-lg bg-destructive/10 hover:bg-destructive hover:text-destructive-foreground text-destructive transition-all duration-300 whitespace-nowrap lg:mt-auto text-sm lg:text-base"
              >
              ...
              <div className="space-y-6 animate-fade-in">
                ...
                <Card className="p-6 border-2">
                  ...
                  <form onSubmit={createOnlineClass} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <Input placeholder="لینک کلاس" value={newClass.link} onChange={(e) => setNewClass({ ...newClass, link: e.target.value })} required className="text-right" dir="ltr" />
              ...
              <div className="space-y-6 animate-fade-in">
                ...
                <Card className="p-6 border-2">
                  ...
                  <form onSubmit={createJozveh} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <Input placeholder="عنوان جزوه" value={newJozveh.title} onChange={(e) => setNewJozveh({ ...newJozveh, title: e.target.value })} required className="text-right" />
              ...
              <div className="space-y-6 animate-fade-in">
                ...
                <Card className="p-6 border-2">
                  ...
                  <form onSubmit={handleCreateAkhbar} className="space-y-4">
                    <Input placeholder="عنوان خبر" value={newAkhbar.title} onChange={(e) => setNewAkhbar({ ...newAkhbar, title: e.target.value })} required className="text-right" />
                    <Textarea placeholder="متن خبر (از ** برای بولد کردن استفاده کنید)" value={newAkhbar.content} onChange={(e) => setNewAkhbar({ ...newAkhbar, content: e.target.value })} required className="text-right min-h-[120px]" />
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {GRADE_OPTIONS.map(g => (
                          <Button key={g.value} type="button" variant={newAkhbar.targetGrades.includes(g.value) ? "default" : "outline"} size="sm" onClick={() => toggleGradeInAkhbar(g.value)}>{g.label}</Button>
                        ))}
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      <Switch checked={newAkhbar.isPublished} onCheckedChange={(checked) => setNewAkhbar({ ...newAkhbar, isPublished: checked })} />
                ...
                {editingPish ? (
                  <Card className="p-6 border-2">
                    ...
                    <div className="space-y-4">
                      ...
                      <Textarea placeholder="متن (از ** برای بولد کردن استفاده کنید)" value={pishForm.content} onChange={(e) => setPishForm({ ...pishForm, content: e.target.value })} className="text-right min-h-[120px]" />
                      <div className="space-y-2">
                        ...
                        {(editingPish.image_url || pishImage) && (
                          <img src={pishImage ? URL.createObjectURL(pishImage) : editingPish.image_url!} alt="preview" className="w-full max-h-48 object-contain rounded-lg mt-2" />
                        )}
                      </div>
            ...
            {activeSection === "roles" && (
              <div className="space-y-6 animate-fade-in">
                ...
                <Card className="p-8 border-2 text-center">
                  ...
                  <p className="text-muted-foreground">مدیریت نقش‌ها و دسترسی‌ها در نسخه بعدی اضافه خواهد شد</p>
                </Card>
              </div>
            )}
        ...
        <DialogContent dir="rtl" className="sm:max-w-md">
          ...
          <div className="space-y-4 py-4">
            <div>
              ...
              <Input value={editUserForm.full_name} onChange={(e) => setEditUserForm({ ...editUserForm, full_name: e.target.value })} className="text-right" />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">نام کاربری جدید</label>
              <Input value={editUserForm.username} onChange={(e) => setEditUserForm({ ...editUserForm, username: e.target.value })} className="text-right" placeholder="خالی = بدون تغییر" />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">رمز عبور جدید</label>
              <Input type="password" value={editUserForm.password} onChange={(e) => setEditUserForm({ ...editUserForm, password: e.target.value })} className="text-right" placeholder="خالی = بدون تغییر" />
            </div>
